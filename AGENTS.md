...
6. **Limbi:** cod și identificatori în engleză; textele din interfață în română, **doar** în `strings.xml`, niciodată scrise direct în composable-uri; textele generate pentru client sau contabil în italiană, doar în `domain/Descrizione.kt` și `domain/Dictionary.kt`.
...
7. **Logica de business stă în `domain/`**, pură, testabilă fără Android. Zero logică de calcul în composable-uri.
8. Fără `!!`, fără `GlobalScope`, fără operații de bază de date pe main thread.
9. Nicio dependență nouă fără justificare. Dependențele se declară **doar** în `gradle/libs.versions.toml`.
...
- **M5 — gata, de verificat local:** `domain/Totals.kt` (baza după felul plății, extra acceptate și facturabile, total, de facturat, rest de încasat), încasări și evidența facturilor pe lucrare, ecranul Bani cu cele trei cifre mari și lista „De facturat”. Teste noi: `TotalsTest` 9. Schema nu se schimbă: `payments` și `invoices` existau din M1, s-au adăugat doar interogări de citire.
- **Urmează M6:** „Descriere pentru factură” în italiană, `domain/Dictionary.kt` (SPEC §15), testul golden byte cu byte și legătura din detaliul lucrării către banii ei.
